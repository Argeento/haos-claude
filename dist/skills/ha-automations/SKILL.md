...
1. Before saving — **always validate** YAML: `ha core check`
2. After editing `automations.yaml` — **reload**:
   - `ha core restart` (full restart) or
   - Via API: `POST /api/services/automation/reload` (faster, no Core restart)
3. Before major changes — **backup**: `ha backups new --name "before-changes"`
